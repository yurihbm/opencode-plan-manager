...
test("generatePlanId - handles different plan types", () => {
  ...
  expect(generatePlanId("feature", "Login", date)).toBe("feature_login_20260206");
  expect(generatePlanId("bug", "Login", date)).toBe("bug_login_20260206");
  expect(generatePlanId("refactor", "Login", date)).toBe("refactor_login_20260206");
...
test("generatePlanId - handles long multi-word titles", () => {
  ...
  expect(id).toBe("refactor_plugin-refactor-folder-per-plan-architecture_20260206");
});
