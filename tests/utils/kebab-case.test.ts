...
test("toKebabCase - underscores and hyphens", () => {
  ...
  expect(toKebabCase("___multiple___underscores___")).toBe("multiple-underscores");
});
...
test("toKebabCase - real-world examples", () => {
  ...
  expect(toKebabCase("Feature/Add-Payment-Gateway")).toBe("feature-add-payment-gateway");
  expect(toKebabCase("v2.0 Migration (Phase 1)")).toBe("v2-0-migration-phase-1");
});
