...
describe("ensurePlanDirectories", () => {
	test("creates all 3 status directories", async () => {
		...
		await Bun.file(paths.pending)
			.exists()
			.catch(() => false);
		await Bun.file(paths.in_progress)
			.exists()
			.catch(() => false);
		await Bun.file(paths.done)
			.exists()
			.catch(() => false);
		...
		expect(readdir(paths.pending)).resolves.toBeDefined();
		expect(readdir(paths.in_progress)).resolves.toBeDefined();
		expect(readdir(paths.done)).resolves.toBeDefined();
	});
