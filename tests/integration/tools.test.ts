...
test("Integration: Full workflow — create → list → read → update status → update task → done", async () => {
  ...
  const createResult = await plugin.tool.plan_create.execute(
    {
      ...
      spec: {
        ...
        nonFunctionals: ["Must handle 1000 concurrent users", "Response time under 200ms"],
        acceptanceCriterias: ["Login validates credentials", "Token refresh extends session"],
        ...
        phases: [
          {
            ...
            tasks: ["Create auth middleware", "Build login endpoint", "Add token refresh"],
          },
        ],
      },
    },
  ...
  expect(
    await Bun.file(join(inProgressPath, "metadata.json")).exists(),
  ).toBe(true);
  expect(await Bun.file(join(pendingPath, "metadata.json")).exists()).toBe(
    false,
  );
  ...
  const result = await plugin.tool.plan_create.execute(
    {
      ...
      spec: {
        overview: "Migrate database from SQLite to PostgreSQL with zero downtime",
      ...
      implementation: {
        ...
        phases: [
          {
            phase: "Phase 1: Setup",
            tasks: ["Set up PostgreSQL", "Write migration scripts", "Test rollback"],
          },
        ],
      },
    },
...
test("Integration: Update with taskContent (legacy) fails", async () => {
  ...
  const result = await plugin.tool.plan_update.execute(
    { plan_id: planId, taskContent: "Some task" } as any,
    context,
  );
  ...
  const result = await plugin.tool.plan_update.execute(
    {
      ...
      plan: {
        ...
        phases: [
          { phase: "Phase 1", tasks: ["Task A", "Task A"] },
        ],
      },
    },
    context,
